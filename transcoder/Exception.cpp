...
av::Exception::Exception(const char* msg)
{
    buffer = msg;
}

av::Exception::Exception(const std::string& msg)
{
    buffer = msg.c_str();
}

void av::ExceptionHandler::ck(int ret)
{
    if (ret < 0) throw Exception("an AV exception has occurred");
}

void av::ExceptionHandler::ck(int ret, CmdTag cmd_tag)
{
    if (ret < 0) {
        char av_str[256];
...
void av::ExceptionHandler::ck(int ret, const std::string& msg)
{
    if (ret < 0) {
        char av_str[256];
        av_strerror(ret, av_str, 256);
...
void av::ExceptionHandler::ck(void* arg, CmdTag cmd_tag)
{
...
const av::Exception av::ExceptionHandler::getNullException(CmdTag cmd_tag)
{
...
void av::ExceptionHandler::msg(const std::string& msg, MsgPriority priority, const std::string& qualifier)
{
    if (fnMsgOut)
...
const char* av::ExceptionHandler::tag(CmdTag cmd_tag)
{
    switch (cmd_tag) {
    case CmdTag::AO2:
